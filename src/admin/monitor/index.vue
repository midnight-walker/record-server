<template>
    <layout class="body-content" :nav-active="nav">
        <el-row class="search-bar">
            ...
            <el-button type="primary" icon="plus" @click="handleImport">导入监测点</el-button>
            <el-button type="primary" icon="plus" @click="handleExport">导出监测计划表</el-button>
        </el-row>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            handleDelete(row) {
                this.$confirm('此操作将永久删除该条监测记录, 是否继续?', '提示', {
                    type: 'warning'
                }).then(() => {
                    this.$ajax({
                        method: 'DELETE',
                        url: '/api/monitor/' + row.id
                    }).then(res => {
            ...
            handleExport(){
                ...
                window.location.href='/api/monitor/export?projectId='+this.query.projectId;
            }
        }
    }
