/**
 * Created by tqj <2482366539@qq.com> on 2019/2/14.
 */
let {defaultHeadStyle,defaultBodyStyle,highlightStyle} = require('../../../utils/export/base/style');
//用于处理超过Z列的数据
    ...
    if (body) {
        data = body
            .map((v, i) => columns.map((k, j) => {
                let value=v[k['prop']],useHighlight=v[k['prop']+'highlight'];
                if(!value && value!==0){
                    value='';
                }
                return Object.assign({}, {
                    v: value,
                    s: Object.assign({}, defaultBodyStyle, k.bodyStyle || {},(v['highlight'] || useHighlight)?k.highlightStyle:{}),
